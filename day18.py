...
def part1_solve(input_data: str, grid_size: int, steps: int) -> int:
    ...
    g = nx.Graph()
    max_y_bounds = input_grid.shape[0] - 1
    max_x_bounds = input_grid.shape[1] - 1
    start_node = f'0,0'
    end_node = f'{max_y_bounds},{max_x_bounds}'

    for (y, x), input_value in np.ndenumerate(input_grid):
        if input_value == wall:
            continue
        current_node = f'{y},{x}'
        ...
        for move in moves:
            ...
            if next_x < 0 or next_x > max_x_bounds:
                continue
            next_pos_value = input_grid[next_y][next_x]
...
def main() -> None:
    ...
    if "6,1" == ic(part2_solve(example_input)):
